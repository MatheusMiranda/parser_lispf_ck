...
lexer = ox.make_lexer([
    ('NAME',r'^(?!;|def|loop|doBefore|doAfter|sub|dec|add|inc|print)[a-zA-Z]+'),
    ('NUMBER', r'\d+'),
...
tokens_list = ['NUMBER', 'FUNCTION_DECLARATION', 'EXECUTION_BLOCK', 'READ_INPUT',
     ...
     'ADD_OPERATOR','SUB_OPERATOR','NAME', 'LOOP_OPERATOR']
...
function_declaration = lambda do, function_name, block: ('function_declaration', do, function_name, block)
loop_block_operator = lambda loop, block : ('loop_structure', loop, block)

parser = ox.make_parser([
    ('block : FUNCTION_DECLARATION atom block', function_declaration),
    ...
    ('simple_block : LOOP_OPERATOR simple_block', loop_block_operator),
